...
public class CDPlayer extends MusicPlayer {
	...
	public void nextTrack() {
		...
		if (thisTrack >= this.playlist.size()
				|| thisTrack >= this.PLAYLIST_SIZE) {
	...
	public void previousTrack() {
		thisTrack--;
		if (thisTrack >= this.playlist.size()
				|| thisTrack >= this.PLAYLIST_SIZE || thisTrack < 0) {
			System.out
					.println("This is the first song! No More previous songs! ");
			thisTrack = 0;
			return;
		}
	...
	public void play(int trackNumber) {
		...
		if (trackNumber >= this.playlist.size()
				|| trackNumber >= this.PLAYLIST_SIZE || trackNumber < 0) {
